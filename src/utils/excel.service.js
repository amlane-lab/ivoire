...
let ExcelService = class ExcelService {
    constructor(uploadService, utilService) {
    ...
    async exportProducts(products, categories, categoriesWithNoSubCat, userId, userService) {
        ...
        for (var i = 0, l = categories.length; i < l; i++) {
            ...
            if (tempArr[cat.categoryId])
                tempArr[cat.categoryId].list.push({ id: cat._id, title: cat.title });
            else
                tempArr[cat.categoryId] = { id: cat.categoryId, title: cat.categoryName, list: [{ id: cat._id, title: cat.title }] };
        }
        ...
        for (var key in tempArr) {
            ...
            await obj.list.forEach(async (ele, index) => {
                if (index == 0)
                    await catSheet.addRow({ id: obj.id, title: obj.title, subCategoryId: ele.id.toString(), subCategoryName: ele.title });
                else
                    await catSheet.addRow({ subCategoryId: ele.id.toString(), subCategoryName: ele.title });
            });
            await catSheet.addRow();
        }
    ...
    async createImportTemplate(categories, categoriesWithNoSubCat) {
        var workbook = new Excel.Workbook();
        let sheet = workbook.addWorksheet("Products");
        sheet.columns = await this.getHeaders();
        let catSheet = workbook.addWorksheet("Categories");
    ...
    async importProducts(file, categories, categoriesWithNoSubCat) {
        try {
            var workbook = new Excel.Workbook();
